...
struct dma_link* ptr_dma_list =  &dma_link_list;
...
int16 MT9V034_CFG[CONFIG_FINISH][2]=
{
    ...
    {EXP_TIME,          400}, //曝光时间          摄像头收到后会自动计算出最大曝光时间，如果设置过大则设置为计算出来的最大曝光值
    ...
    {GAIN,              37},  //图像增益          范围16-64     增益可以在曝光时间固定的情况下改变图像亮暗程度
...
void SmartCar_MT9V034_Init(void)
{
    ...
    link_list_num = SmartCar_EruDma_Init(MT9V034_DMA_CH, GET_PORT_IN_ADDR(0*32), camera_buffer_addr, MT9V034_PCLK_PIN, FALLING, MT9V034_W*(MT9V034_H-34));//引脚P00_0
...
//        mysbfunction();
//        Ifx_DMA* dma = dma_link_list.channel.dma;
//        IfxDma_ChannelId dma_ch = dma_link_list.channel.channelId;
//        IfxDma_resetChannel(dma_link_list.channel.dma,dma_link_list.channel.channelId);
//        (dma_link_list).channel.dma;
//        (dma_link_list).channel.channelId;
       // link_list_num = SmartCar_EruDma_Init(MT9V034_DMA_CH, GET_PORT_IN_ADDR(0*32), camera_buffer_addr, MT9V034_PCLK_PIN, FALLING, MT9V034_W*MT9V034_H);
...
{
    image_size_half_forOLED((uint8*)image, (uint8*)image_size_half, 120, 188);

    dis_bmp(ro / 2, co / 2, (uint8*)image_size_half, 100);
}
void SmartCar_Show_IMG_WITHOUT_TH(uint8* image, uint16 ro, uint16 co)
{
    image_size_half_forOLED((uint8*)image, (uint8*)image_size_half, 120, 188);

    dis_IMG(ro / 2, co / 2, (uint8*)image_size_half);
}
