...
void MENU_Init()//存取数据时最后一个数据不能操作，待解决
{
    ...
    search_line = MENU_fileInit(search_line, 50, 1.0, "line", 5, dataint, &endGyro, &parkDelay, NULL, NULL);
...
//    currentKD = MENU_fileInit(currentKD, 2, 3.0, "motorKD", 4, datafloat, &currentRTKI, &expectC, NULL, NULL);
    ...
    presentServoD = MENU_fileInit(presentServoD, 1, 4.5, "preServoD", 6, datafloat, &fuzzyNB, &gap, NULL, NULL);
...
//    presentMotorP = MENU_fileInit(gap, 8, 250, "preMotorP", 3, datafloat, &presentMotorI, &bottomData, NULL, NULL);
    ...
    display9 = MENU_fileInit(display9, 68, 9.02, "param1", 4, dataint, &display8, &display10, NULL, NULL);
