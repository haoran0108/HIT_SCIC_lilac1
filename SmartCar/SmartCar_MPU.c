...
#ifdef SMARTCAR_SMARTCAR_SPI_H_
...
{
    uint8 data[2];
    SmartCar_MPU_Readregs(ICM20602_DEV_ADDR, GYRO_ZOUT_H, data, 2);
    ...
    my_mpu->mpu_rawdata.gyro_z = my_mpu->mpu_config.gyro_inv * (short int)(((short int)data[0]<<8) | data[1]) - my_mpu->offset_data.gyro_z;
}
